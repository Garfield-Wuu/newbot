...
int main(int argc, char **argv) {
  ...
  if (ldlidar_drv->WaitLidarComm(3500)) {
    ...
    ROS_INFO("ldlidar communication is abnormal.");
    //exit(EXIT_FAILURE); //不要异常退出 edit by newbot
  }
  ...
  while (ros::ok() && 
    ...
    switch (ldlidar_drv->GetLaserScanData(laser_scan_points, 1500)){
      ...
      case ldlidar::LidarStatus::DATA_TIME_OUT: {
        //ROS_ERROR("point cloud data publish time out, please check your lidar device."); //关闭报错 edit by newbot
        break;
      }
  ...
  if (lidar_spin_freq > 0) {
    ...
    for (auto point : src) {
      ...
      if (index < beam_size) {
        ...
        index = (index + beam_size/2) % beam_size;//----------------旋转180度 edit by newbot
